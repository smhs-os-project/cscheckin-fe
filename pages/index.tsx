    ...
    <BasePage id="home" title="首頁" full>
      ...
      {school === "" ? (
        <div className="select-school">
          <h2 className="text-2xl">選擇學校</h2>
          <div className="school-options grid grid-flow-col grid-cols-1 md:grid-cols-6 gap-3">
            {availableSchool.map((org) => (
              <div key={`available-school-option-${org}`} className="w-max">
                <BaseButton type="button" onClick={() => setSchool(org)}>
                  {org}
                </BaseButton>
              </div>
            ))}
